...
이전 구현에서 생성된 bootstrap credential이나 최초 변경 flag가 아직 없는 기존 설치의 내부 관리자
credential을 AAA-JDBC가 인증 중 예외로 처리하는 호환성 상황에서는, SSO가 보호 대상 관리자와 대화형
로그인 조건을 확인한 경우에만 인증 session 없이 credential-change challenge로 진행한다. 실제 변경은
AAA의 `CREDENTIALS_CHANGE`가 이전 암호를 다시 검증해야 성공하며, 실패하거나 비대화형 요청이면
session/token을 발급하지 않는다.
...
* `Unexpected Exception invoking: AAA_AUTHN_AUTHENTICATE_CREDENTIALS`가 발생하면 SSO의 최초 변경 상태를
  임의로 해제하지 않는다. 보호 대상 내부 관리자의 대화형 요청이면 SSO는 flag가 없는 이전 설치도
  session을 발급하지 않고 변경 challenge로 전환한다. AAA가 이전 암호를 검증하여 변경에 성공해야만
  로그인 복구가 가능하다. 그 외 요청은 계속 실패하고 같은 시각의 `Extension invocation failed` log를
  조사한다.
