...
신규 Engine DB를 생성하는 `engine-setup`에서 입력한 `admin@internal` 암호는 운영용 영구 암호가 아니라 **bootstrap 암호**로 취급한다. setup은 AAA-JDBC에 암호를 저장할 때 유효 종료시각을 과거로 지정한다. 따라서 사용자가 해당 암호로 WebAdmin에 처음 인증하면 AAA는 `CREDENTIALS_EXPIRED`를 반환하고 SSO는 일반 WebAdmin session을 발급하지 않은 채 암호 변경 화면으로 유도한다.
...
AAA-JDBC가 `CREDENTIALS_EXPIRED`를 반환하면 `AuthnMessageMapper`는 해당 profile이 암호 변경 URL 또는 `CREDENTIALS_CHANGE` capability를 제공하는지 확인한다. 지원하면 SSO의 “암호가 만료되었습니다. 계속하려면 암호를 변경하십시오” 경로를 사용한다. 변경 요청은 `AuthenticationService.changePassword()`가 이전 credential과 새 credential을 AAA extension의 `CREDENTIALS_CHANGE` command로 전달한다.
...
| 방식 | 판정 | 설명 |
|---|---|---|
| 초기 AAA-JDBC 암호를 만료 상태로 생성 | **채택** | 기존 SSO 만료·암호변경 protocol을 재사용하고 일반 session 전에 강제 가능 |
| 최초 로그인 후 WebAdmin popup으로 변경 권고 | 부적합 | popup 우회·닫기가 가능하고 session이 이미 발급될 수 있음 |
...
5. bootstrap 암호 입력 후 dashboard가 아니라 암호 변경 화면으로 이동하는지 확인한다.
6. 이전 암호와 새 암호를 입력하고 변경을 완료한다.
...
| ID | 시험 | 기대결과 |
|---|---|---|
| FLPC-01 | 신규 DB engine-setup 완료 후 bootstrap 암호로 WebAdmin 로그인 | 일반 dashboard 진입 전 암호 변경 요구 |
| FLPC-02 | 변경 화면을 취소하고 WebAdmin URL 직접 호출 | 인증된 관리 session 없음, 다시 로그인/변경 요구 |
...
| 로그인 화면의 만료 암호 변경 link | `backend/manager/modules/enginesso/src/main/webapp/WEB-INF/login.jsp` |
